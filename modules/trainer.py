        ...
        with tqdm(total=self.n_train, desc=f'Epoch {epoch + 1}/{self.max_epoch}', unit='img') as pbar:
            for batch in self.train_loader:
                ...
                if self.global_step % (self.n_dataset // (10 * self.batch_size)) == 0:
                    ...
                    #### dv: using a batch_size > 1 affects saving the temporary checkpoint outputs
                    #### where a single .png is horizontally concatenated with all side outputs, labeled
                    #### image and input echogram. So, to fix this, we can just export the first image
                    #### in the batch in the temporary checkpoint
                    #### Fix:
                    if self.batch_size > 1:
                        first_images = [batch_tensor[0, 0] for batch_tensor in outputs]
                        outputs = first_images
                        label=label[0]
                        image=image[0]
                    #### 
